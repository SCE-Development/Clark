...
function init () {
  var mdbiAccessToken =
    '0d08441c35cd57bf9a639821fd7a49b6e195e4bdee642dac0478b8f994b8ec32' // stored in credentials.json
  // setDebug(true)
    ...
    if (collectionName === '') {
      ...
      try {
        newDoc =
          $('#jsonInputField').val() === ''
            ? {}
            : JSON.parse($('#jsonInputField').val())
      } catch (err) {
        console.log(
          `Error: failed to parse raw JSON data "${$('#jsonInputField')}"`
        )
        console.log(err)
        showError(err)
        return
  ...
  $('#testMongoSearch').on('click', function (event) {
    ...
    console.log(
      `Finding ${
        data === null ? 'all collections' : JSON.stringify(data)
      } in database...`
    )
  ...
  $('#testMongoDocSearch').on('click', function (event) {
    ...
    if (collectionName === '') {
      ...
      try {
        searchCriteria =
          $('#dbDocRawJsonField').val() === ''
            ? {}
            : JSON.parse($('#dbDocRawJsonField').val())
      } catch (err) {
        console.log(
          'Error: failed to parse raw JSON data "' +
            $('#dbDocRawJsonField').val() +
            '"'
        )
        console.log(err)
      ...
      if (
        $('#fieldProjectionToggle')[0].checked === true &&
        $('#fieldProjectionText').val() !== ''
      ) {
        try {
          ...
          console.log(
            'Error: failed to parse projection JSON data "' +
              $('#fieldProjectionText').val() +
              '"'
          )
          console.log(err)
      ...
      if (
        $('#resultSetLimitToggle')[0].checked === true &&
        $('#resultSetLimitText').val() !== ''
      ) {
        try {
          if (typeof data.options === 'undefined') {
            data.options = {}
          ...
          console.log(
            `Error: failed to parse result set size limit "${$(
              '#resultSetLimitText'
            ).val()}"`
          )
          console.log(err)
      ...
      // Gather Page Options
      if (
        $('#resultSetPageToggle')[0].checked === true &&
        $('#resultSetPageText').val() !== ''
      ) {
        try {
          if (typeof data.options === 'undefined') {
            data.options = {}
          ...
          console.log(
            `Error: failed to parse result set page number "${$(
              '#resultSetPageText'
            ).val()}"`
          )
          console.log(err)
      ...
      if (
        $('#resultSetSortToggle')[0].checked === true &&
        $('#resultSetSortSpec').val() !== ''
      ) {
        try {
          if (typeof data.options === 'undefined') {
            data.options = {}
          ...
          console.log(
            `Error: failed to parse sort spec JSON "${$(
              '#resultSetSortSpec'
            ).val()}"`
          )
          console.log(err)
  ...
  $('#testMongoDocDelete').on('click', function (event) {
    ...
    if (collectionName === '') {
      ...
      try {
        searchCriteria =
          $('#dbDocDeleteRawJsonField').val() === ''
            ? {}
            : JSON.parse($('#dbDocDeleteRawJsonField').val())
      } catch (err) {
        console.log(
          'Error: failed to parse raw JSON data "' +
            $('#dbDocDeleteRawJsonField').val() +
      ...
      post(
        deleteQuantity === 'Many'
          ? '/mdbi/delete/documents'
          : '/mdbi/delete/document',
        data,
  ...
  $('#testMongoDocUpdate').on('click', function (event) {
    ...
    if (collectionName === '') {
      ...
      try {
        searchCriteria =
          $('#dbDocUpdateRawJsonFilterField').val() === ''
            ? {}
            : JSON.parse($('#dbDocUpdateRawJsonFilterField').val())
        updateCriteria =
          $('#dbDocUpdateRawJsonUpdateField').val() === ''
            ? {}
            : JSON.parse($('#dbDocUpdateRawJsonUpdateField').val())
      } catch (err) {
        console.log(
          'Error: failed to parse raw JSON data from either update or search criteria'
        )
        console.log(err)
        showError(err)
        return
...
function showError (err) {
  console.log(
    `Logging ${typeof err === 'object' ? JSON.stringify(err) : err} to screen`
  )
  switch (typeof err) {
    case 'string': {
    ...
    case 'object': {
      try {
        ...
        $('#content_panel').html(
          '<h3>Error</h3>' + '<p>' + errObjAsString + '</p>'
        )
      } catch (err) {
        $('#content_panel').html(
          '<h3>Error</h3>' +
            '<p>Attempted to convert error object to string, but failed: ' +
    ...
    default: {
      $('#content_panel').html(
        '<h3>Error</h3>' + '<p>An unknown error occurred...</p>'
      )
      break
    }
  }
...
function showCollectionResults (arr) {
  if (arr.length > 0) {
    var accordionHeader =
      "<div id='fCR_Accordion' class='panel-group' role='tablist' aria-multiselectable='true'>"
    ...
    for (var i = 0; i < arr.length; i++) {
      ...
      content += `<li class="list-group-item"><strong>NS:</strong> ${
        isView ? 'n/a' : arr[i].idIndex.ns
      }</li>`
      content += '</ul>'
      content += '</div>'
      content += '</div>'
...
function showDocumentResults (arr) {
  if (arr.length > 0) {
    var accordionHeader =
      "<div id='fDR_Accordion' class='panel-group' role='tablist' aria-multiselectable='true'>"
    var accordionFooter = '</div>'
    var content = accordionHeader // prepend content with starting div tag
    ...
    for (var i = 0; i < arr.length; i++) {
      ...
      content += `<li class="list-group-item"><div><strong>Top-Level Keys:</strong> ${Object.keys(
        arr[i]
      ).toString()}</div><code style="overflow-wrap: break-word">${
        typeof arr[i] === 'object' ? JSON.stringify(arr[i]) : arr[i]
      }</code></li>`
      content += '</ul>'
      content += '</div>'
      content += '</div>'
...
function showSingleDeleteResults (result) {
  var content = `<h3>Deletion Results</h3><p>${
...
function showSingleUpdateResults (result) {
  var content = `<h3>Update Results</h3><p>${
    typeof result === 'object' ? JSON.stringify(result) : result
  }</p>`
  $('#content_panel').html(content)
}
...
function showSingleWriteResults (result) {
  var content = `<h3>Write Results</h3><p>${
    typeof result === 'object' ? JSON.stringify(result) : result
  }</p>`
  $('#content_panel').html(content)
}
