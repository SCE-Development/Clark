...
angular.module("doorcoder").component("doorcoder", {
	...
	"controller": function ($http, $timeout) {
		...
		this.requestDoorCodeList = function () {
			...
			$http.post(links.doorCodeGetAll, requestBody, config).then((response) => {
				...
				switch (status) {
					case 200: {
						...
						if (typeof result === "undefined") {
							...
							ctl.setErrorMessage(`Error: Expected array, got ${typeof result}`);
						} else {
					...
					case 499: {
						ctl.setErrorMessage((typeof result[0].emsg === "undefined") ? "Session Token Rejected..." : result[0].emsg);
						break;
					}
					case 500: {
						ctl.setErrorMessage((typeof result[0].emsg === "undefined") ? "An internal server error occurred..." : result[0].emsg);
						break;
					}
					default: {
				...
				console.log(`[DoorCoderController] requestDoorCodeList -> Problem: ${errResponse}`);
			});
		};
		...
		this.submitNewDoorCode = function () {
			...
			$http.post(links.doorCodeEdit, requestBody, config).then((response) => {
				var result = response.data;
				var status = response.status;
				var isArray = Array.isArray(result);
				var hasEmsg = (!isArray) ? false : (typeof result[0].emsg === "undefined") ? false : true;
				...
				switch (status) {
					...
					case 499: {
						ctl.setErrorMessage((hasEmsg) ? result[0].emsg : "Error: A core error occurred");
						break;
					}
					case 500: {
						ctl.setErrorMessage((hasEmsg) ? result[0].emsg : "Error: An internal server error occurred");
						break;
					}
					default: {
						ctl.setErrorMessage(`Unexpected Response (Code ${status})`);
				...
				try {
					...
					var hasEmsg = (!hasData) ? false : (typeof errResponse.data.emsg === "undefined") ? false : true;
					...
					ctl.setErrorMessage("A client-side error occurred: JSON.stringify()");
				}
				console.log(`[DoorCoderController] submitNewDoorCode -> Problem: ${msg}`);
			});
		};
