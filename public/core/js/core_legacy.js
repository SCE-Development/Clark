		...
		post(uri, {"user":credentials.user, "pwd": credentials.pwd}, function (response, status, jqxhr) {
			$scope.$apply(function () {
				...
				if (typeof response.etype !== "undefined") {
					...
					// });
					$http.post(credentials.redirect, requestBody, config).then(function (response) {
...
loginController.$inject = ["$scope", "$window", "$location", "$http", "pageData"];
...
var pageController = function ($scope, pageData) {
	$scope.$watch(function () {return pageData.get("html");}, function (newVal, oldVal) {
		if (newVal !== oldVal) {
