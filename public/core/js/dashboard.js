/* global storageAvailable, $, setDebug, angular, sessionStorage, logDebug */
// PROJECT:   Core-v4
//  Name:    Rolando Javier
//  File:    dashboard.js
...
// Globals
const storageOk = storageAvailable('sessionStorage')
...
// BEGIN AngularJS App
const pageApp = angular.module('adminDashboard', [
  'profiler',
  'officertools',
  'admintools'
])
...
*/
pageApp.controller('UserController', function userController (
  $scope,
  $http,
  $window
) {
  ...
  $scope.logout = function () {
    const requestBody = {
      sessionID: storageOk
        ...
        : 'Without session storage, cookies should be automatically sent to the server',
    ...
    $http
      .post(urls.logout, requestBody, config)
      .then(response => {
        // called when http status code is within the 200s
        switch (response.status) {
          case 200: {
            // logout succeeded; go to core login page
      ...
      .catch(function (errResponse) {
        // called when http status code is outside 200s
        logDebug(
          'UserController',
          'logout',
          `Error: ${JSON.stringify(errResponse.data)}`
        )
      })
  }
})
