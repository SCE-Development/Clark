...
//     the command "node db_setup.js [option]", where constious options can be used.
...
const fs = require('fs')
const SchemaManager = require('../schemaManager/schemaManager') // acquire schemaManager class
...
const sm = new SchemaManager() // initialize a schema manager
...
const url = `mongodb://${encodeURIComponent(
  ...
  mongoSettings.port
}/${mongoSettings.database}?authSource=${mongoSettings.database}`
...
function formatDatabase () {
  ...
  mongo.connect(url, mongoOptions, function (err, db) {
    ...
    if (err) {
      ...
      console.log(
        `WARNING: You're about to delete all records in the ${mongoSettings.database} database! Are you sure? (Yes/no)`
      )
      process.stdin.on('readable', function () {
        ...
        if (chunk !== null) {
          ...
          if (answer.includes('no')) {
            ...
            const collectionNames = sm.getSchemaNames()
              ...
              const tempPromise = new Promise(function (resolve, reject) {
                ...
                db.dropCollection(name, null, function (err, result) {
                  ...
                  if (err) {
                    ...
                    // Print result
                    console.log(
                      `Collection ${name} dropped?: ${JSON.stringify(result)}`
                    )
            ...
            console.log(
              `Unexpected answer "${answer}". Please say "Yes" or "no"...`
            )
          }
        }
      })
...
function initDatabase (mock = false) {
  ...
  mongo.connect(url, mongoOptions, function (err, db) {
    ...
    if (err) {
      ...
      const schemas = sm.getSchemaDefinitions() // views are also represented as schemas
      ...
      Object.keys(schemas).forEach(function (schemaName) {
        ...
        if (schemaDef.type === 'view') {
          ...
          viewPromises.push(getViewCreatorPromise(schemaDef, db))
        } else {
          // Generate a collection creation promise and store it
          schemaPromises.push(
            getCollectionCreatorPromise(
              schemaDef,
              db,
        ...
        if (mock && Array.isArray(schemaDef.mock)) {
          ...
          mockPromises.push(
            getCollectionMockPromise(sm.getSchema(schemaName), db)
          )
        }
      })
              ...
              if (mockPromises.length > 0) {
                ...
                Promise.all(mockPromises)
                  .then(function (message) {
                    // End with success
                    console.log(
                      `Successfully initialized database "${mongoSettings.database}" with mock documents...`
                    )
                    endSession(db)
                  })
                  .catch(function (error) {
                    // End with failure
                    console.log(
                      `Successfully initialized database "${mongoSettings.database}", but encountered errors when inserting mock documents...`
                    )
                    console.log(error)
                    if (db) endSession(db)
                  })
                ...
                // End with success
                console.log(
                  `Successfully initialized database "${mongoSettings.database}"...`
                )
                endSession(db)
              }
            })
        ...
        if (schemaDef.type === 'collection' && schemaDef.textIndex) {
          ...
          defineTextIndex(db, schemaDef, function (err, res) {
            ...
            if (err) {
              // End with error
              console.log(
                `Created collection "${schemaDef.name}" (${result}), but failed to create text-index "${schemaDef.textIndex.iname}" (${err})`
              )
            } else {
              // End with success
              console.log(
                `Created collection "${schemaDef.name}" (${result}) with text-index "${schemaDef.textIndex.iname}" (${res})`
              )
              resolve()
            }
          })
    ...
    if (conforms) {
      ...
      db.collection(schema.name).insertMany(schema.mock, function (err, res) {
        ...
        if (err) {
          // End with failure
          console.log(
            `Failed to insert mock documents for collection "${schema.name}": ${err}`
          )
          reject(new Error(err))
        } else {
          ...
          autoIncrement(
            ...
            function (error, result) {
              ...
              if (error) {
                // End with failure
                console.log(
                  `Inserted mock documents for collection "${schema.name}" (${res}), but failed to update auto-increment record (${error})`
                )
                reject(new Error(error))
              } else {
                // End with success
                console.log(
                  `Inserted mock documents for collection "${schema.name}" (${res}) and updated auto-increment record (${result})`
                )
                resolve()
              }
            },
      ...
      // End with failure
      console.log(
        `Error: Conformity check failed for 1 or more mock documents in collection "${schema.name}"`
      )
      reject(new Error())
    }
  })
...
function help () {
  ...
  msg += '\n\t--mock'
  msg += '\n\t\tSame as "--init", but adds constious "fake" documents to the'
