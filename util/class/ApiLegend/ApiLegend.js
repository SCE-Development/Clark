...
class ApiLegend {
  ...
  constructor (template) {
    ...
    this.router =
      typeof template.router === 'undefined' ? undefined : template.router
    this.template =
      typeof template.template === 'undefined'
        ? defaultTemplate
        : template.template
    this.endpoints = []
  }
...
// @returns   n/a
ApiLegend.prototype.register = function (
  name,
  method,
  route,
  desc,
  ...
  const handlerTag = { src: 'ApiLegend.register' }
...
ApiLegend.prototype.getDoc = function (pretty = false) {
  ...
  switch (pretty) {
    ...
    case true: {
      ...
      const docTemplate = fs.readFileSync(this.template, 'utf-8')
