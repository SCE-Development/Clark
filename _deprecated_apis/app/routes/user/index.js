...
apiInfo.args.login = [
  ...
  },
  {
    ...
    type: 'boolean',
    desc:
      'A boolean signifying whether or not the client browser supports the modern ' +
...
apiInfo.rval.login = [
  {
    ...
    desc:
      'A code 200, and a sessionID to validate all server operations during the session'
  },
  {
...
api.register(
  ...
  function (request, response) {
    ...
    const handlerTag = { src: '(post) /api/user/login' }
    ...
    const grantCoreAccess = function (match, resolve, reject) {
      ...
      www.https.post(memberUpdateOptions, memberUpdateBody, function (
        ...
        if (error) {
          ...
          response
            .status(200)
            .send(rf.asCommonStr(true, sessionResponse))
            .end()
          resolve()
        }
      })
...
apiInfo.args.logout = [
  ...
  },
  {
    ...
    type: 'boolean',
    desc:
      "A boolean indicating the client browser's support of sessionStorage technology"
  }
]
apiInfo.rval.logout = [
  {
    condition: 'On success',
    desc:
      'a code 200, and a success message indicating the user logged out successfully'
  },
  {
    condition: 'On failure',
...
api.register(
  ...
  apiInfo.rval.logout,
  function (request, response) {
    const handlerTag = { src: '(post) /api/user/logout' }
    const validBody =
    ...
    if (!validBody) {
      ...
      const uname =
        typeof request.body.userName !== 'string' ? null : request.body.userName
      const sid =
        typeof request.body.sessionID !== 'string'
          ? null
...
apiInfo.args.add = [
  ...
  },
  {
    ...
    type: 'object',
    desc:
      'A JSON object representing the user to add to the database. This must contain ' +
  ...
  },
  {
    ...
    type: '~Date',
    desc:
      "The new user's membership plan expiration date. If omitted, this defaults to " +
  ...
  },
  {
    ...
    type: '~Date',
    desc:
      "The new user's start date for Core systems/Member systems access. If " +
  ...
  },
  {
    ...
    type: '~Date',
    desc:
      "The new user's expected graduation date. If omitted, this defaults to null."
  },
  {
    name: 'request.newUser.emailOptIn',
    type: '~string',
    desc:
      'A boolean specifying whether the user wants to receive emails from us. This ' +
      'defaults to false.'
  },
  {
  ...
  },
  {
    ...
    type: '~number',
    desc:
      "The dcId of the new user's door code. If omitted, a doorcode will not be " +
  ...
  },
  {
    ...
    type: '~number',
    desc:
      'The id of the clearance level to assign to the new user. If omitted, this ' +
...
apiInfo.rval.add = [
  ...
  },
  {
    ...
    desc:
      'A code 200, and an error format object describing the the existence of the user'
  },
  {
    condition: 'On failure',
...
api.register(
  ...
  apiInfo.rval.add,
  function (request, response) {
    const handlerTag = { src: '(post) /api/user/add' }
    const sessionID =
      typeof request.body.sessionID !== 'undefined'
        ? request.body.sessionID
        : null
    const userID =
      typeof request.body.userID !== 'undefined' ? request.body.userID : null
    const newUser =
      typeof request.body.newUser !== 'undefined' ? request.body.newUser : null
    let session = null // houses session data of the user to verify
    ...
    const capabilityCallback = function (result) {
      ...
      switch (result) {
        ...
        case false: {
          ...
          const msg =
            `User with memberID "${session.memberID}" not permitted` +
            ' to perform member addition'
          logger.log(msg, handlerTag)
          response
            .status(200)
            .send(ef.asCommonStr(ef.struct.adminUnauthorized, { msg: msg }))
            .end()
          break
        }
        case true: {
    ...
    const aggregationCallback = function (reply, error) {
      ...
      if (error) {
        ...
        const userAlreadyExists = reply.length > 0
    ...
    const insertionCallback = function () {
      ...
      www.https.post(options, body, function (reply, error) {
        ...
        if (error) {
          ...
          const newUserMembershipData = {
            ...
            doorCodeID:
              typeof newUser.doorcodeID === 'undefined'
                ? false
                : newUser.doorcodeID,
            gradDate:
              typeof newUser.gradDate === 'undefined'
                ? false
                : newUser.gradDate,
          ...
          www.https.post(options, body, function (reply, error) {
            ...
            if (error) {
              ...
              const errStr = ef.asCommonStr(ef.struct.httpsPostFail, error)
              logger.log(
                `Ne user membership registration failed: ${errStr}`,
                handlerTag
              )
              response
                .status(500)
...
apiInfo.args.delete = [
  ...
  },
  {
    ...
    type: 'string',
    desc:
      'The URL parameter to place after the "/delete", which represents the member ' +
...
api.register(
  ...
  function (request, response) {
    ...
    const handlerTag = { src: '(post) /api/user/search' }
    const sessionID =
      typeof request.query.sessionID !== 'undefined'
        ? request.query.sessionID
        : null
    const idToDelete =
      typeof request.query.id !== 'undefined' ? request.query.id : null
    ...
    au.verifySession(credentials.mdbi.accessToken, sessionID, function (
      ...
      if (error) {
        ...
        au.isCapable([2], sessionData.memberID, function (result) {
          ...
          if (result === -1) {
            // If error, report error
            const msg = 'Error: Capability check failed'
            logger.log(msg, handlerTag)
            response
              .status(500)
              .send(ef.asCommonStr(ef.struct.coreErr, { msg: msg }))
              .end()
          } else if (!result) {
            // If the client is not capable, report it
            const msg = `Member id ${sessionData.memberID} not permitted to delete users`
            logger.log(msg, handlerTag)
            response
              .status(200)
              .send(
    ...
    const deleteMembershipData = function (userID) {
      ...
      www.https.post(options, body, function (reply, error) {
        ...
        if (error) {
          ...
          if (reply.ok === 0) {
            ...
            logger.log(
              `Delete MembershipData Error: Mongo replied with NACK (${reply})`,
              handlerTag
            )
            response
              .status(500)
            ...
            logger.log(
              'Delete MembershipData Error: No document was deleted',
              handlerTag
            )
            response
              .status(500)
    ...
    const deleteMember = function (userID) {
      // Prepare request parameters
      const body = {
        accessToken: credentials.mdbi.accessToken,
        collection: 'Member',
      ...
      www.https.post(options, body, function (reply, error) {
        ...
        if (error) {
          ...
          if (reply.ok === 0) {
            // If the db didn't acknowledge your request, report it
            logger.log(
              `Delete Member Error: Mongo replied with NACK (${reply})`,
              handlerTag
            )
            response
              .status(500)
            ...
            // If the delete didn't do anything, reply with an error
            logger.log(
              'Delete Member Error: No document was deleted',
              handlerTag
            )
            response
              .status(500)
...
apiInfo.args.search = [
  ...
  },
  {
    ...
    type: 'string',
    desc:
      'The type of search to execute. Currently supported search types include ' +
  ...
  },
  {
    ...
    type: '~object',
    desc:
      'An optional JSON object to customize the result set after it is returned from ' +
  ...
  },
  {
    ...
    type: '~boolean',
    desc:
      'A boolean specifying whether or not to interpret the searchTerm as a regular ' +
      'expression'
  },
  {
...
apiInfo.args.edit = [
  ...
  },
  {
    ...
    type: 'object',
    desc:
      "A JSON object specifying the fields to edit. The object's keys can include " +
...
apiInfo.rval.edit = [
  ...
  },
  {
    condition: 'On invalid/expired session token',
    desc:
      'a code 499, and an error-formatted object in the response body detailing the ' +
      'expired session issue'
  },
  {
    condition: 'On illegal/unsuccessful update',
    desc:
      'a code 200, and an error-formatted object in the response body detailing the ' +
      'invalid operation'
  },
  {
    condition: 'On any other failure',
...
api.register(
  ...
  function (request, response) {
    ...
    const handlerTag = { src: '(post) /api/user/edit' }
    const sessionID =
      typeof request.body.sessionID !== 'undefined'
        ? request.body.sessionID
        : null
    const memberID =
      typeof request.body.memberID !== 'undefined'
        ? request.body.memberID
        : null
    const fields =
      typeof request.body.fields !== 'undefined' ? request.body.fields : null
    let session = null
    ...
    const updateQueue = organizeUpdates(fields)

    // Structure the update queries for both Member and MembershipData collections
    const queries = buildEditQueries(memberID, updateQueue)

    // Create a Promise to update data in the MemberCollection
    const updateMC = new Promise(function (resolve, reject) {
      // Send requests
      logger.log(
        ...
        www.https.post(
          ...
          function (reply, error) {
            ...
            if (error) {
              ...
              const efmt = ef.asCommonStr(ef.struct.mdbiNoEffect, {
                unmodified: {
                  collection: 'Member',
              ...
              const efmt = ef.asCommonStr(ef.struct.mdbiMultiEffect)
              logger.log(efmt, handlerTag)
              // reject({
              //   statusCode: 499,
              //   responseMsg: JSON.parse(efmt)
    ...
    const updateMdC = new Promise(function (resolve, reject) {
    ...
    const capabilityCallback = function (result) {
      ...
      switch (result) {
        ...
        case false:
          ...
          logger.log(
            `User with memberID "${session.memberID}" not permitted` +
              ' to perform member data modification',
            handlerTag
          )
          response
            .status(499)
        ...
        case true:
          ...
          Promise.all([updateMC, updateMdC]).then(
            function (valOnSuccess) {
              // If both promises resolve, then no serious errors occurred. Check for
              // warnings
              const warnings = []
              const successes = []
              for (let i = 0; i < valOnSuccess.length; i++) {
              ...
              if (successes.length === 0) {
                ...
                response
                  .status(200)
                  .send(
                    ef.asCommonStr(ef.struct.mdbiPartialEffect, {
                      unmodified: warnings,
                      modified: successes
                    })
                  )
            ...
            function (valOnFailure) {
              ...
              response
                .status(valOnFailure.statusCode)
                .send(valOnFailure.responseMsg)
                .end()
            }
          )
          break
...
function organizeUpdates (fields) {
  ...
  const memberFields = Object.keys(schema.collectionMembers.Member)
  const membershipDataFields = Object.keys(
...
function buildEditQueries (memberID, updateQueue) {
  ...
  return {
    ...
    options: {
      ...
      MembershipData:
        updateQueue.MembershipData.length > 0 ? mdQueryOptions : false
    }
  }
}
