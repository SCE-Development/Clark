...
describe('<UserNavbar />', () => {
	...
	it('Should render two <NavLink /> components for' + " the user who isn't a member", () => {
		const wrapper = mount(<UserNavbar {...nonMemberAppProps} />);
		expect(wrapper.find(NavLink)).to.have.lengthOf(2);
	});
	it('Should render one <UncontrolledDropdown /> tag for' + ' the unauthenticated user', () => {
		const wrapper = mount(<UserNavbar />);
		expect(wrapper.find(UncontrolledDropdown)).to.have.lengthOf(1);
	});
	it('The one <UncontrolledDropdown /> tags for' + ' the unauthenticated user should be to join sce ', () => {
		const wrapper = mount(<UserNavbar />);
		const dropdowns = wrapper.find(UncontrolledDropdown);
		expect(getDropdownDetails(dropdowns.get(0))).to.equal('Join Us!');
	});
	it('Should render two <UncontrolledDropdown /> tags for' + ' the authenticated user', () => {
	...
	it(
		...
		() => {
			...
			expect(getDropdownDetails(dropdowns.get(1), 1)[0].props.children).to.equal('Profile');
		}
	);
});
