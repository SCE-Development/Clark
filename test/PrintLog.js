...
describe('PrintLog', () => {
  before(done => {
    app = tools.initializeServer(__dirname + '/../api/logging/PrintLog.js');
  ...
  describe('/POST addPrintLog', () => {
    it("Should return 400 when required fields aren't filled in", 
    async () => {
      const result = await test.sendPostRequest(
        '/api/PrintLog/addPrintLog', INVALID_PRINT_LOG);      
    ...
    async () => {
      const result = await test.sendPostRequest(
        '/api/PrintLog/addPrintLog', VALID_PRINT_LOG);
