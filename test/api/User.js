...
describe('User', () => {
  ...
  describe('/POST register', () => {
    ...
    it('Should return 200 even if audit logging fails', async () => {
      const logAuditStub = sinon.stub(AuditUtil, 'logAudit').throws(new Error('Audit logging failed'))
      try {
        const res = await test.sendPostRequest('/api/Auth/register', {
          ...registerPayload,
        ...
        const auditEntry = await AuditLog.findOne({'details.email': 'anotherEmail@example.com'}).lean()
        expect(auditEntry).to.not.exist
