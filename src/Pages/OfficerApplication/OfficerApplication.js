...
class OfficerApplication extends Component {
  ...
  async handleSubmit(e) {
    ...
    const response = await addToSpreadsheet(row, sheetsId);
    if (response.error) {
      this.setState({
        validSpreadsheetId: false
      });
    }
    this.setState({
      submitted: true
  ...
  render() {
    ...
    } else if (this.state.submitted && this.state.validSpreadsheetId) {
      return (
        <div id="wrapper">
          <p>
						Thank you for applying for an SCE officer position!
            Your application has been submitted. Please
						wait 1-2 days for a confirmation email with next steps.
          </p>
        </div>
      );
    } else {
          ...
          <div id="wrapper" onSubmit={this.handleSubmit}>
            ...
            <form id="form-area">
              ...
              <div className="form-group">
                ...
                <h5>
								Share a link to your resume (pdf, docx, doc)
                using <strong> Google Drive</strong>.
                </h5>
                <input
                  type="url"
                  placeholder="https://"
