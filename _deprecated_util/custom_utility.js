...
cutil.delintRequestBody = function (body, callback) {
  const handlerTag = { src: 'cutil.delintRequestBody' }
...
*/
cutil.hashString = function (unhashedString) {
  ...
  for (let i = 0; i < unhashedString.length; i++) {
    // Get the character we'll be appending
    let c = unhashedString[i]
    // If it's a letter...
    if (c.match(/[a-z]/i)) {
      // Get its code
      const code = unhashedString.charCodeAt(i)
      // Uppercase letters
      if (code >= 65 && code <= 90) {
