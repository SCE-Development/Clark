...
//  Details:
//      This file contains custom cryptography functions used throughout the Core v4 system to encrypt constious credentials, settings, etc.
//  Dependencies:
//      node_hash v0.2.0
...
cryptic.hashPwd = function (username, password) {
  const salt = username + (username.length % 12).toString() // some simple salt
...
cryptic.hashSessionID = function (username) {
  const salt = new Date(Date.now()).toISOString()
