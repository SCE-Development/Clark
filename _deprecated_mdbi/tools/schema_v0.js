 ...
 */
  collectionNames: [
    'serverActivations',
    'autoIncrements', // a collection for auto_incrementing constious collection attributes
    'Member',
    'MembershipData',
    'DoorCode',
    'ClearanceLevel',
 ...
 */
  ...
  */
    addCollection: function (name, members, callbk) {
      let error = null
      // let result = null
  ...
  */
    conformsWith: function (name, doc) {
      const format = schema.collectionMembers[name]
      ...
      if (Object.keys(doc).length !== formatKeys.length) {
        ...
        for (let i = 0; i < formatKeys.length; i++) {
          if (typeof doc[formatKeys[i]] !== typeof format[formatKeys[i]]) {
            success = false
            break
          }
        }
