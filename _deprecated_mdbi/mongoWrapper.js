...
const schema = require('./tools/schema_v0') // import database schema
const logger = require(`${settings.util}/logger`) // import event log system
const ef = require(`${settings.util}/error_formats`) // import error formatting system
const cu = require(`${settings.util}/custom_utility`) // import custom utilities
...
/*
    @member     mdb.Database
    @details    A link to our MongoDB Database; initially set to null, so you'll have to initiate a mongodb connection externally before associating this constiable with a database
*/
mdb.database = null
// END Members
...
mdb.insertDoc = function (collection, doc, callback, options = undefined) {
  const handlerTag = { src: 'insertDoc' }
  const modifiers = typeof options !== 'undefined' ? options : {}

  // Check if database collection exists
  mdb.database.collection(collection, { strict: true }, function (
    error,
    result
  ) {
    if (error != null) {
      ...
      logger.log(
        `Error looking up collection "${collection}": ` + error.toString(),
        handlerTag
      )
      ...
      autoIncrement(collection, function (error, incrementVal) {
        ...
        if (error) {
          ...
          if (
            typeof schema.ppk[collection] !== 'undefined' &&
            modifiers.preserveKey !== true
          ) {
            // Set the ppk (preferred primary key) of this document to the value of the docId
            logger.log(
              `Overwriting ppk using __docId__ ${doc.__docId__}...`,
              handlerTag
            )
          ...
          logger.log(
            `Writing new document ${typeof doc} ${
              typeof doc === 'object' ? JSON.stringify(doc) : doc
            }`,
            handlerTag
          )
...
mdb.findCollections = function (collectionName, callback) {
  const handlerTag = { src: 'findCollections' }
  const searchCriteria =
    collectionName !== undefined && typeof collectionName === 'string'
      ? { name: collectionName }
      : undefined
  ...
  switch (searchCriteria === undefined) {
    ...
    case false: {
      logger.log(
        `Returning search result for collection ${searchCriteria.name}`,
        handlerTag
      )
...
/*
    ...
    @returns    n/a
    @details    This function searches for documents fitting the filter criteria if given, or all documents in the collection if not (i.e. given an empty JSON object). The filter criteria is a JSON object which (if not empty) is expected to contain constious parameters:
        {
            "someParam0": "criteria0",
            ...
            "someParamN": "criteriaN"
        }
    The parameters and criteria given in the object will consty depending on the structure of the collection you are searching.
...
mdb.findDocs = function (collection, filter, callback, constraints) {
  ...
  mdb.database.collection(collection, { strict: true }, function (
    ...
    if (error != null) {
      ...
      let queryFail = false
      let limitMsg = 'unlimited'
      let pageMsg = 'unpaginated'
      let cursor = result.find(filter)
      ...
      if (typeof constraints === 'object') {
        ...
        if (!queryFail && typeof constraints.projection !== 'undefined') {
          ...
          const keys = Object.keys(constraints.projection)
          for (let i = 0; i < keys.length; i++) {
            if (typeof constraints.projection[keys[i]] !== 'number') {
              // Number conversion failed; reply with error
              logger.log(
                'Error: failed to convert projection values to numbers',
                handlerTag
              )
              eobject = ef.asCommonStr(ef.struct.convertErr, {
                parameter: 'constraints.projection'
              })
              queryFail = true
              break
            }
        ...
        if (!queryFail && typeof constraints.limit !== 'undefined') {
          ...
          if (typeof constraints.limit !== 'number') {
            ...
            logger.log(
              'Error: failed to convert limit constraint to number',
              handlerTag
            )
            eobject = ef.asCommonStr(ef.struct.convertErr, {
              parameter: 'constraints.limit'
            })
            queryFail = true
          } else {
            ...
            const pageType = typeof constraints.page
            if (pageType !== 'undefined') {
              if (pageType !== 'number') {
                // Conversion failed; respond with an error
                logger.log(
                  'Error: failed to convert page constraint to number',
                  handlerTag
                )
                eobject = ef.asCommonStr(ef.struct.efconvertErr, {
                  parameter: 'constraints.page'
                })
                queryFail = true
              } else {
        ...
        if (!queryFail && typeof constraints.sort === 'object') {
          const sortOrders = [] // make an empty array of sort specs
          const ascKeys = constraints.sort.asc
          const descKeys = constraints.sort.desc
          ...
          if (!Array.isArray(ascKeys) && !Array.isArray(descKeys)) {
            ...
            eobject = ef.asCommonStr(ef.struct.unexpectedValue, {
              parameter: 'constraints.sort'
            })
            queryFail = true
          } else {
            // Compile ascending sort specs
            if (Array.isArray(ascKeys)) {
              for (let i = 0; i < ascKeys.length; i++) {
      ...
      if (!queryFail) {
        logger.log(
          `Finding ${limitMsg} docs matching ${typeof filter} ${
            typeof filter === 'object' ? JSON.stringify(filter) : filter
          } in collection "${collection}" (${pageMsg})`,
          handlerTag
        )
...
mdb.aggregateDocs = function (collection, aggregation, callback, agOptions) {
  const handlerTag = { src: 'aggregateDocs' }
  ...
  mdb.database.collection(collection, { strict: true }, function (
    ...
    if (error) {
      ...
      if (typeof callback === 'function') {
        const errStr = ef.asCommonStr(ef.struct.invalidDataType, {
      ...
      // Perform aggregation
      // const customOptions = (typeof agOptions === 'undefined') ? null : agOptions
      logger.log(
        `Finding docs in collection "${collection}" using the Aggregation Pipeline`,
        handlerTag
      )
      result.aggregate(aggregation, function (err, list) {
        if (err !== null) {
          logger.log(
            `Error performing aggregation: ${err.toString()}`,
            handlerTag
          )
...
/*
    ...
    The filter criteria is a JSON object which is expected to contain constious parameters:
        {
            "someParam0": "criteria0",
            ...
            "someParamN": "criteriaN"
...
mdb.findDocCount = function (collection, filter, callback) {
  const handlerTag = { src: 'findDocCount' }
...
mdb.deleteOneDoc = function (collection, filter, callback) {
  const handlerTag = { src: 'deleteOneDoc' }
  ...
  mdb.database.collection(collection, { strict: true }, function (
    ...
    if (error != null) {
      ...
      logger.log(
        `Deleting first doc matching ${typeof filter} ${
          typeof filter === 'object' ? JSON.stringify(filter) : filter
...
mdb.deleteManyDocs = function (collection, filter, callback) {
  const handlerTag = { src: 'deleteManyDocs' }
  ...
  mdb.database.collection(collection, { strict: true }, function (
    ...
    if (error != null) {
      ...
      logger.log(
        `Deleting all docs ${
          hasFilter
            ? 'matching ' + typeof filter + ' ' + (typeof filter === 'object')
...
/*
    ...
    @returns    n/a
    @details    This function searches through the specified collection for the FIRST document matching the search criteria specified by filter. If found, it then updates the document in the way described by the update object (i.e. via constious MongoDB update operators). Then, the function runs the callback, regardless of a successful or failed update operation.
...
mdb.updateOneDoc = function (collection, filter, update, callback) {
  const handlerTag = { src: 'updateOneDoc' }
  const hasFilter = !(
    filter === {} ||
    filter === null ||
    filter === undefined ||
  ...
  if (!hasFilter || !hasUpdate) {
    logger.log(
      'Error before updating: Operation ineffective with undefined filter and update objects',
      handlerTag
    )
    if (typeof callback === 'function') {
      callback(
  ...
  mdb.database.collection(collection, { strict: true }, function (
    ...
    if (error != null) {
      ...
      logger.log(
        `Updating first doc matching ${typeof filter} ${filter} in collection "${collection}"`,
        handlerTag
      )
...
(number) result - the current value of the collection's auto-increment record
