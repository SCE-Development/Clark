...
const settings = require('../../util/settings') // acquire server system settings
const logger = require(`${settings.util}/logger`) // acquire event log system
const mdb = require('../mongoWrapper') // acquire MongoDB API Wrappers
const credentials = require(settings.credentials) // acquire db auth credentials
const ef = require(`${settings.util}/error_formats`) // acquire error formatter
const mongoSettings = require('../mongo_settings') // acquire MongoDB settings
const schema = require('../tools/schema_v0') // acquire current db schema
// END Includes
...
)}:${encodeURIComponent(credentials.mdbi.pwd)}@${mongoSettings.hostname}:${
  mongoSettings.port
}/${mongoSettings.database}`
mongo.connect(url, mongoOptions, function (err, db) {
  const handlerTag = { src: '/mdbi' }
    ...
    mdb.findCollections(null, function (error, list) {
      ...
      for (let i = 0; i < schema.collectionNames.length; i++) {
        if (listOfNames.indexOf(schema.collectionNames[i]) === -1) {
          logger.log(
            `Error: Collection ${schema.collectionNames[i]} not found`,
            handlerTag
          )
          endSession(db)
      ...
      const postmark = logger.log('Database connection established', handlerTag)
      mdb.insertDoc('serverActivations', { login: postmark }, function (
        error,
        result
      ) {
        if (error != null) {
...
router.post('/write', function (request, response) {
  const handlerTag = { src: 'mdbi/write' }
  const hasBody = Object.keys(request.body).length > 0
  const searchCriteria = hasBody
    ? numerify(delintRequestBody(request.body))
    : {}
  ...
  if (!checkAuth(request.body.accessToken)) {
    ...
    response
      .send(ef.asCommonStr(ef.struct.mdbiAccessDenied))
      .status(499)
      .end()
  } else {
    logger.log(
      `Client @ ip ${
        request.ip
      } is requesting to write a new document of ${typeof searchCriteria.data} ${
        typeof searchCriteria.data === 'object'
          ? JSON.stringify(searchCriteria.data)
          : searchCriteria.data
      } to the ${searchCriteria.collection} collection in the database`,
      handlerTag
    )
    mdb.insertDoc(
      ...
      function (error, result) {
        ...
        if (error != null) {
          ...
          response
            .status(200)
            .send(typeof result === 'object' ? JSON.stringify(result) : result)
            .end()
        }
      },
...
router.post('/search/collections', function (request, response) {
  const handlerTag = { src: 'mdbi/search/collections' }
  const hasBody = Object.keys(request.body).length > 0
  const searchCriteria =
    hasBody && typeof request.body.name !== 'undefined'
      ? delintRequestBody(request.body).name
      : ALL_COLLECTIONS
  ...
  if (!checkAuth(request.body.accessToken)) {
    ...
    logger.log(
      `Client @ ip ${request.ip} is requesting to find ${
        searchCriteria === ALL_COLLECTIONS
          ? 'all collections'
          : 'collection ' + JSON.stringify(searchCriteria)
      } from the database`,
      handlerTag
    )
...
router.post('/search/documents', function (request, response) {
  const handlerTag = { src: 'mdbi/search/documents' }
  const hasBody = Object.keys(request.body).length > 0
  const hasOptions = !hasBody ? false : typeof request.body.options === 'object'
  const searchCriteria = hasBody
    ? numerify(delintRequestBody(request.body))
    : {} // either the filter, or empty JSON
  ...
  if (!hasBody) {
    ...
    const projection = !hasOptions
      ? null
      : typeof request.body.options.projection === 'object'
        ? request.body.options.projection
        : null
    const limit = !hasOptions
      ? null
      : typeof request.body.options.limit !== 'undefined'
        ? request.body.options.limit
        : null
    const page = !hasOptions
      ? null
      : typeof request.body.options.page !== 'undefined'
        ? request.body.options.page
        : null
    const sortSpec = !hasOptions
      ? null
      : typeof request.body.options.sort !== 'undefined'
        ? request.body.options.sort
        : null
    const queryCallback = function (error, list) {
      if (error != null) {
        logger.log('An error occurred', handlerTag)
        response
          .status(500)
    ...
    logger.log(
      `Client @ ip ${request.ip} is requesting to find ${
        searchCriteria === {}
          ? 'all documents'
          : 'documents matching "' +
            (typeof searchCriteria.search === 'object'
              ? JSON.stringify(searchCriteria.search)
              : searchCriteria.search) +
            '"'
      } from the ${searchCriteria.collection} collection in the database`,
      handlerTag
    )

    // Compile constraints
    let constraints =
      limit === null && page === null && sortSpec === null ? null : {}
    if (limit !== null) {
...
router.post('/search/aggregation', function (request, response) {
  const handlerTag = { src: 'mdbi/search/aggregation' }

  const hasBody = Object.keys(request.body).length > 0
  const searchCriteria = hasBody
    ? numerify(delintRequestBody(request.body))
    : null
  const hasCollectionName = !hasBody
    ? false
    : typeof searchCriteria.collection !== 'undefined'
  // const hasAgOptions = (!hasBody) ? false : typeof searchCriteria.agOptions !== 'undefined'
  const hasAccessToken = !hasBody
    ? false
    : typeof searchCriteria.accessToken !== 'undefined'
  let failure = false
  let errMsg = ''
  let errObj = 'Err'
  let errCode = 0
  ...
  if (failure) {
    ...
    // const agOpts = (searchCriteria.agOptions === null) ? null : searchCriteria.agOptions

    // Perform query
    try {
      logger.log(
        `Client @ ip ${request.ip} is requesting to find documents via aggregation from the ${searchCriteria.collection} collection in the database`,
        handlerTag
      )
      mdb.aggregateDocs(
        searchCriteria.collection,
        searchCriteria.pipeline,
        queryCallback
      )
    } catch (e) {
...
router.post('/delete/document', function (request, response) {
  const handlerTag = { src: 'mdbi/delete/document' }
  const hasBody = Object.keys(request.body).length > 0
  const searchCriteria = hasBody
    ? numerify(delintRequestBody(request.body))
    : {}
  ...
  if (!checkAuth(request.body.accessToken)) {
    ...
    logger.log(
      `Client @ ip ${request.ip} is requesting to delete a document matching ${
        typeof searchCriteria.search === 'object'
          ? JSON.stringify(searchCriteria.search)
          : searchCriteria.search
      } from the ${searchCriteria.collection} collection in the database`,
    ...
    mdb.deleteOneDoc(searchCriteria.collection, searchCriteria.search, function (
      error,
      result
    ) {
      if (error != null) {
...
router.post('/delete/documents', function (request, response) {
  const handlerTag = { src: 'mdbi/delete/documents' }
  const hasBody = Object.keys(request.body).length > 0
  const searchCriteria = hasBody
    ? numerify(delintRequestBody(request.body))
    : {}
  ...
  if (!checkAuth(request.body.accessToken)) {
    ...
    logger.log(
      `Client @ ip ${request.ip} is requesting to delete all documents ${
        searchCriteria.search === {}
          ? ''
          : 'matching ' +
            (typeof searchCriteria.search === 'object'
...
router.post('/update/documents', function (request, response) {
  const handlerTag = { src: 'mdbi/update/documents' }
  const hasBody = Object.keys(request.body).length > 0
  const searchCriteria = hasBody
    ? numerify(delintRequestBody(request.body))
    : {}
  ...
  if (!checkAuth(request.body.accessToken)) {
    ...
    logger.log(
      `Client @ ip ${
        request.ip
      } is requesting to update a document matching ${typeof searchCriteria.search} ${
        typeof searchCriteria.search === 'object'
          ? JSON.stringify(searchCriteria.search)
          : searchCriteria.search
      } from the ${
        searchCriteria.collection
      } collection in the database using ${typeof searchCriteria.update} ${
        typeof searchCriteria.update === 'object'
          ? JSON.stringify(searchCriteria.update)
...
router.use(function (request, response) {
  const handlerTag = { src: 'mdbi/NOTFOUND' }
  logger.log(
...
router.use(function (err, request, response) {
  // const handlerTag = { src: 'mdbi/ERROR' }
...
function delintRequestBody (body, callback) {
  const handlerTag = { src: 'delintRequestBody' }
  logger.log(`Delinting ${JSON.stringify(body)}`, handlerTag) // Debug
  const newBody = body
  const newBodyKeys = Object.keys(newBody) // array of keys
  for (let i = 0; i < newBodyKeys.length; i++) {
    const qmarkIndex = newBodyKeys[i].indexOf('?')
    if (qmarkIndex !== -1) {
      logger.log(
        `Lint found at ${newBodyKeys[i]} index ${qmarkIndex}`,
        handlerTag
      ) // Debug
      // If the key has a "?", create a new property without it.
      const newKeyName = newBodyKeys[i].substring(qmarkIndex + 1) // acquires the string without "?" in it
...
function numerify (obj) {
  ...
  try {
    ...
    const newObj = JSON.parse(objAsString, (key, value) => {
      const handlerTag = { src: 'bodyParser.json.Reviver' }
      // Attempt to convert string into number
      if (
        typeof value === 'string' &&
        Number.isNaN(Number(value)) === false &&
        value.match(/0x.*|[bB][01]*|.*[0123456789].*/gi) !== null
      ) {
        logger.log(
          `Converting string ${value} into number ${Number(value)}`,
          handlerTag
        )
        return Number(value)
      } else {
        logger.log(
          `Leaving ${typeof value} ${
    ...
    logger.log(
      `Unable to numerify ${typeof obj} ${JSON.stringify(obj)}:\n${err}`,
      handlerTag
    )
    return obj
  }
...
/*
 ...
 @details  This function takes an array of JSON objects "arr" and iterates through it, extracting from each object the value associated with "key" and placing it into an array, which is either passed to the callback parameter (if defined), or returned to the caller. If the "arr" contains objects that do not have the same set of keys, any objects that do not have the member "key" will cause this function to populate the corresponding array index with "null". As an example, running this section of code:
  const myArrayOfObjects = [{"a":0, "b":1}, {"a":2, "b":3}, {"a":4, "c":5}, {"a":6, "b":7}];
  const theKeyToFind = "b"
  const result = extractFromObjectArray(myArrayOfObjects, theKeyToFind);
...
//   for (const i = 0; i < unhashed_string.length; i++) {
//     // Get the character we'll be appending
//     const c = unhashed_string[i]
//     // If it's a letter...
//     if (c.match(/[a-z]/i)) {
//       // Get its code
//       const code = unhashed_string.charCodeAt(i)
//       // Uppercase letters
...
//       ptrIndex = i
//
//       const compareVal = ((options.msMode === true) ? arr[ptrIndex].total : arr[ptrIndex])
//
...
//         const pivotIndex = partition(arr, start, end, options)
...
//   for (const i = Math.floor(arr.length / 2) - 1; i >= 0; i--) {
...
//   const largestChildIndex = index
//   const leftChildIndex = lchild(index)  //   const rightChildIndex = rchild(index)  //
...
//    largestChildIndex = rightChildInde  //  }
...
//   const smallestChildIndex = index
//   const leftChildIndex = lchild(index)
//   const rightChildIndex = rchild(index)
//
//   // Look for largest child between index and its left child
