...
const smci = require('../smci/smci')
const smciSettings = require('../smci/smci_settings')
const www = require('../util/www.js')
...
describe('SMCI Unit Tests', function () {
    ...
    describe('API Root', function () {
      ...
      describe('smci.api.ping()', function () {
        it('should ping the api server health', function (done) {
          ...
          const expectedKeys = ['health_status']
  ...
  if (ctl.runAll || ctl.runAuthorizedApps) {
    describe('Authorized Apps', function () {
      describe('smci.authorizedApps.register()', function () {
        it(
          "shouldn't be tested yet, until you're certain it won't override MailChimp's client ids"
        )
      })
      describe('smci.authorizedApps.getFullList()', function () {
        it('should acquire a full list of authorized apps from MailChimp', function (done) {
          const expectedKeys = ['apps', 'total_items', '_links']
      ...
      describe('smci.authorizedApps.getAppInfo()', function () {
        it('should acquire information about a specific app', function (done) {
          const expectedKeys = ['id', 'name', 'description', 'users', '_links']
          const appID = '394135694205' // Client id associated with the MailChimp account's app "rj_my_test_app"
          smci.authorizedApps.getAppInfo(appID, null, function (
            response,
            error
          ) {
            assert.isNull(error)
    ...
    describe('Automations', function () {
      describe('smci.automations.getFullList()', function () {
        it('should acquire a full list of automations from MailChimp', function (done) {
          const expectedKeys = ['automations', 'total_items', '_links']
  ...
  if (ctl.runAll || ctl.runBatchOps) {
    describe('Batches', function () {
      describe('smci.batchOps dummy test', function () {
        it(
          "is a dummy test, since I don't yet know how to test Batch Operations..."
        )
      })
  ...
  if (ctl.runAll || ctl.runCampaignFolders) {
    describe('Campaign Folders', function () {
      let campaignFolderId = '' // nothing yet
      const campaignFolderName = 'smci_unit_test_folder'
      const newFolderName = 'smci_unit_test_new_folder'

      describe('smci.campaignFolders.createFolder()', function () {
        it('should create a campaign folder in MailChimp', function (done) {
          const expectedKeys = ['name', 'id', 'count', '_links']
          smci.campaignFolders.createFolder(campaignFolderName, function (
            response,
            error
          ) {
            assert.isNull(error)
      ...
      describe('smci.campaignFolders.getFullList()', function () {
        it('should acquire a full list of campaign folders from MailChimp', function (done) {
          const expectedKeys = ['folders', 'total_items', '_links']
      ...
      describe('smci.campaignFolders.getFolder()', function () {
        it('should acquire information about the folder we previously created', function (done) {
          const expectedKeys = ['name', 'id', 'count', '_links']
          smci.campaignFolders.getFolder(campaignFolderId, null, function (
            response,
            error
          ) {
            assert.isNull(error)
      ...
      describe('smci.campaignFolders.editFolder()', function () {
        it('should change the campaign folder name', function (done) {
          const expectedKeys = ['name', 'id', 'count', '_links']
          smci.campaignFolders.editFolder(
            campaignFolderId,
            newFolderName,
      ...
      describe('smci.campaignFolders.deleteFolder()', function () {
        it('should delete the folder we created/updated', function (done) {
          ...
          smci.campaignFolders.deleteFolder(campaignFolderId, function (
            response,
            error
          ) {
            assert.isNull(error)
  ...
  if (ctl.runAll || ctl.runCampaigns) {
    describe('Campaigns', function () {
      // const testCampaignID = 'e6e3985d78' // id of a campaign existing in the MailChimp Account; will reassign after creating the campaign
      // const testCampaignListID = '9e8ae49d3e' // id of a list existing in the MailChimp Account
      // const testCopyCampaignID = '' // will populate after copying a campaign
      ...
      //    "recipients",
      //    "settings",
      //    // "constiate_settings", // situational
      //    "tracking",
      ...
      describe('smci.campaigns.getCampaignInfo()', function () {
        it('should acquire info about the created campaign', function (done) {
          ...
          const testCampaignID = 'e6e3985d78' // id of a campaign existing in the MailChimp Account; will reassign after creating the campaign
          smci.campaigns.getCampaignInfo(testCampaignID, null, function (
            response,
            error
          ) {
      ...
      //   const keysOfAnErrorResponse = ["detail", "instance", "status", "title", "type"];
      ...
      describe('smci.campaigns.pauseCampaign()', function () {
        // only works with an RSS campaign
        it(
          "shouldn't be tested unless you create your own RSS campaign using SMCI"
        )
      ...
      describe('smci.campaigns.resumeCampaign()', function () {
        // only works with an RSS campaign
        it(
          "shouldn't be tested unless you create your own RSS campaign using SMCI"
        )
      ...
      //    "instance"
      //   ];
      //   const recipients = ["rjavier443@gmail.com", "rjavier.engr@gmail.com"]; // insert your test email recipients here...
      //   const requestBody = {
      ...
      //   const keysOfAnErrorResponse = ["type", "title", "status", "detail", "instance"];
  ...
  if (ctl.runAll || ctl.runLists) {
    describe('Lists', function () {
      const testListName = 'smci_test_list'
      const testListContact = {
      ...
      const testListPermissionReminder =
        'You subscribed to this email because you agreed to help test the development of SCE Core v4'
      const testListCampaignDefaults = {
        from_name: smciSettings.anystring,
        from_email: smciSettings.accountEmail,
        subject: 'SMCI Test',
        language: 'English'
      }
      let testListID = null // will assign after list creation
      let testListNewMemberID = null // will assign after member addition
      ...
      describe('smci.lists.getFullList()', function () {
        it('should acquire a full list of mailing lists', function (done) {
          const expectedKeys = ['lists', 'total_items', '_links']
      ...
      describe('smci.lists.batchListMembers()', function () {
        it('should add new list members to the created list', function (done) {
          ...
          smci.lists.batchListMembers(testListID, requestBody, function (
            response,
            error
          ) {
            assert.isNull(error)
      ...
      describe('smci.lists.editListSettings()', function () {
        it("should change the created list's settings", function (done) {
          ...
          const requestBody = {
            ...
            contact: testListContact,
            permission_reminder:
              'This is the NEW permission_reminder to remind you that you are signed up for this email since you are helping to test SCE Core v4 (still in development)',
            campaign_defaults: testListCampaignDefaults,
            email_type_option: true
          }

          smci.lists.editListSettings(testListID, requestBody, function (
            response,
            error
          ) {
            assert.isNull(error)
      ...
      describe('smci.lists.addListMember()', function () {
        it('should add a new recipient to the created list', function (done) {
          ...
          smci.lists.addListMember(testListID, requestBody, function (
            response,
            error
          ) {
            assert.isNull(error)
      ...
      describe('smci.lists.getListMembers()', function () {
        it('should acquire a full list of members in the created list', function (done) {
          const expectedKeys = ['members', 'list_id', 'total_items', '_links']

          smci.lists.getListMembers(testListID, null, function (
            response,
            error
          ) {
            assert.isNull(error)
      ...
      describe('smci.lists.removeListMember()', function () {
        it('should remove the list member we added', function (done) {
          ...
          smci.lists.removeListMember(testListID, testListNewMemberID, function (
            response,
            error
          ) {
            assert.isNull(error)
      ...
      describe('smci.lists.searchMembers()', function () {
        it('should list all members fitting the search query', function (done) {
          const expectedKeys = ['exact_matches', 'full_search', '_links']
          const qsObj = {
            query: 'rj'
          }
  ...
  if (ctl.runAll || ctl.runTemplateFolders) {
    describe('Template Folders', function () {
      const createdFolderName = 'SCE SMCI Test Template Folder'
      const createdFolderNewName = 'SCE SMCI Test Other Name Folder'
      let createdFolderID = '' // will reassign after folder creation

      describe('smci.templateFolders.createFolder()', function () {
        it('should create a folder for email templates', function (done) {
          const expectedKeys = ['name', 'id', 'count', '_links']

          smci.templateFolders.createFolder(createdFolderName, function (
            response,
            error
          ) {
            assert.isNull(error)
      ...
      describe('smci.templateFolders.getFolder()', function () {
        it('should acquire info about the created folder', function (done) {
          ...
          smci.templateFolders.getFolder(createdFolderID, null, function (
            response,
            error
          ) {
            assert.isNull(error)
      ...
      describe('smci.templateFolders.editFolder()', function () {
        it('should change the name of the created folder', function (done) {
          ...
          smci.templateFolders.editFolder(
            createdFolderID,
            createdFolderNewName,
            function (response, error) {
              assert.isNull(error)
              assert.containsAllKeys(response, expectedKeys)
              done()
      ...
      describe('smci.templateFolders.deleteFolder()', function () {
        it('should delete the folder we created', function (done) {
          ...
          smci.templateFolders.deleteFolder(createdFolderID, function (
            response,
            error
          ) {
            assert.isNull(error)
  ...
  if (ctl.runAll || ctl.runTemplates) {
    describe('Templates', function () {
      const createdTemplateName = 'SCE SMCI Test Template'
      let createdTemplateID = '' // will reassign after creation of template
      ...
      describe('smci.templates.createTemplate()', function () {
        it('should create a new email template', function (done) {
          ...
          const folderID = null
          const content =
            "<p>This is some random html that I've placed within the email!</p>"

          smci.templates.createTemplate(
            createdTemplateName,
            folderID,
            content,
            function (response, error) {
      ...
      describe('smci.templates.getFullList()', function () {
        it('should acquire a full list of email templates', function (done) {
          const expectedKeys = ['templates', 'total_items', '_links']
      ...
      describe('smci.templates.getTemplateInfo()', function () {
        it('should acquire info about the created template', function (done) {
          ...
          smci.templates.getTemplateInfo(createdTemplateID, null, function (
            response,
            error
          ) {
            // console.log(JSON.stringify(response)); // debug
      ...
      describe('smci.templates.editTemplateName()', function () {
        it('should edit just the name of the template', function (done) {
          ...
          smci.templates.editTemplateName(
            createdTemplateID,
            'some new name',
            function (response, error) {
              assert.isNull(error)
      ...
      describe('smci.templates.moveTemplate()', function () {
        it('should change the location of the template', function (done) {
          ...
          const testFolderID = 'ea54d3c058' // exists as "SCE SMCI Test Templates"
      ...
      describe('smci.templates.deleteTemplate()', function () {
        it('should delete the created template', function (done) {
          ...
          smci.templates.deleteTemplate(createdTemplateID, function (
            response,
            error
          ) {
