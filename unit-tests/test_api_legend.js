...
const al = require('../util/api_legend')
...
describe('API Legend', function () {
  ...
  describe('createLegend()', function () {
    const router = {} // express.js Router emulator

    it('should return an ApiLegend with the given parameters (if any)', function (done) {
      const resultWithoutArgs = al.createLegend()
      const resultWithArgs = al.createLegend('TestName', 'TestDesc', router)

      // Check the type instances of the returned objects
      assert.instanceOf(
        resultWithoutArgs,
        ApiLegend,
        'resultWithoutArgs is an ApiLegend'
      )
      assert.instanceOf(
        resultWithArgs,
        ApiLegend,
        'resultWithArgs is an ApiLegend'
      )
      done()
    })
  })
  ...
  describe('routerRef()', function () {
    const router1 = {} // express.js Router emulator
    const router2 = {} // express.js Router emulator

    it('should return a reference to router1', function (done) {
      const result = al.createLegend('TestName', 'TestDesc', router1)
    ...
    it('should change the reference to router2', function (done) {
      const result = al.createLegend('TestName', 'TestDesc', router2)
  ...
  describe('register()', function () {
    it('should properly register an endpoint', function (done) {
      ...
      const result = al.createLegend('TestName', 'TestDesc', router)
      result.register(
        testRoute.name,
        testRoute.method,
        testRoute.route,
  ...
  describe('getDoc()', function () {
    const docTemplatePath = path.join(
      __dirname,
      '/../util/class/ApiLegend/template/docTemplate.ejs'
    )
    const options = {
    ...
    it('should return a pretty HTML page', function (done) {
      ...
      const result = al.createLegend(data.apiName, data.apiDesc, router)

      // Create your own format string
      const docTemplate = fs.readFileSync(docTemplatePath, 'utf-8')
      const doc = ejs.render(docTemplate, data, options)
