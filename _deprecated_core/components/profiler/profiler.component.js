/* global angular, $, sessionStorage, logDebug */
// PROJECT:   Core-v4
//  Name:    R. Javier
...
angular.module('profiler').component('profiler', {
  ...
  controller: function ($http) {
    ...
    var urls = {
      search: `http://${hostname}/api/user/search`,
      searchMembership: `http://${hostname}/api/user/search`,
      searchDoorCodes: `http://${hostname}/core/dashboard/search/dc`,
      editDoorCodeInAdd: `http://${hostname}/core/dashboard/edit/dc`,
      editMemStatus: `http://${hostname}/core/dashboard/edit/membershipstatus`,
      editMemberField: `http://${hostname}/core/dashboard/edit/memberfield`
    }
    ...
    this.init = function () {
      ...
      $('.member-detail-item')
        .mouseenter(function () {
          $(this).append(
            $(
              "<button type='button' class='btn btn-default member-detail-edit-btn'><span class='glyphicon glyphicon-pencil'></span></button>"
            )
          )
        })
        .mouseleave(function () {
    ...
    this.getEmailVerifiedClassByStatus = function (
      isVerified,
      classIfTrue,
      classIfFalse
    ) {
    ...
    this.viewDetail = function (index) {
      ...
      $http
        .post(urls.searchMembership, requestBody, config)
        .then(response => {
          console.log(response.data.content) // debug
          switch (response.status) {
            case 200: {
              ...
              var startDate = ctl.beautifyDate(
                response.data.content[0].startTerm
              )
              var endDate = ctl.beautifyDate(response.data.content[0].endTerm)
              var memberStatus = response.data.content[0].membershipStatus
                ? 'Active'
                : 'Inactive'
              ...
              $('#memberMiddleInitial').html(
                response.data.content[0].middleInitial
              )
              ...
              $('#memberEmailVerified').addClass(
                response.data.content[0].emailVerified === true
                  ? 'glyphicon-ok'
                  : 'glyphicon-remove'
              )
              $('#memberEmailVerified').css(
                'color',
                response.data.content[0].emailVerified === true
                  ? 'green'
                  : 'red'
    ...
    this.submitNewMemberDetail = function () {
      ...
      if (success) {
        ...
        $http
          .post(urls.editMemberField, requestBody, config)
          .then(response => {
            ...
            switch (response.status) {
              ...
              default: {
                console.log(
                  `Something didn't work... (Code ${response.status})`
                )
          ...
          .catch(function (errResponse) {
            logDebug(
              'ProfilerController',
              'submitNewMemberDetail',
              `Error: ${JSON.stringify(errResponse)}`
            )
            var msg =
              typeof errResponse.data.eobj !== 'undefined'
                ? errResponse.data.eobj
                : typeof errResponse.data.emsg !== 'undefined'
                  ? errResponse.data.emsg
                  : typeof errResponse.data.etype !== 'undefined'
                    ? errResponse.data.etype
                    : 'An error occurred...'
            ctl.showMemberDetailError(msg)
          })
      }
    ...
    this.updateAddMemberModal = function () {
      console.log(
        `Member Registration is at step ${ctl.memberRegistrationStep}`
      )
      ...
      switch (ctl.memberRegistrationStep) {
        case 0: {
          // the default; all except step 0 are hidden
        ...
        default: {
          console.log(
            `Invalid member registration step ${ctl.memberRegistrationStep}`
          )
          break
        }
      }
    ...
    this.finalizeMemberReg = function () {
      ...
      $http
        .post(urls.editMemStatus, requestBody, config)
        .then(response => {
          console.log(response.data.content)
          switch (response.status) {
            case 200: {
    ...
    this.setMemberUsername = function (val) {
      ctl.newMemberUsername =
        typeof val === 'undefined' ? $('#newMemUname').val() : val
    ...
    this.setAddMemberProgress = function (percent) {
      ...
      $('#addMemberProgress')
        .attr('aria-valuenow', ctl.memberRegistrationPercent)
        .css('width', ctl.memberRegistrationPercent)
    }
            ...
            switch (response.status) {
              case 200: {
                ...
                if (response.data.content.length > 1) {
                  ...
                  ctl.showEmailStatus()
                  ctl.showAddMemberError(
                    "Error: The email verification couldn't be checked for some reason..."
                  )
                } else if (typeof response.data.content[0] === 'undefined') {
                  ctl.setEmailStatus('Unknown')
                  ctl.showEmailStatus()
                  ctl.showAddMemberError(
                    "The email you entered returned no search result data. Make sure it's correct!"
                  )
                } else {
                  ...
                  if (!response.data.content[0].emailVerified) {
                    ...
                    ctl.showEmailStatus()
                    ctl.showAddMemberError(
                      "So the email isn't verified... Ask the user to verify their email first!"
                    )
                  } else {
              ...
              case 499: {
                ...
                ctl.showAddMemberError(
                  typeof response.data.content[0].emsg === 'undefined'
                    ? 'Session Token Rejected...'
                    : response.data.content[0].emsg
                )
                break
              }
              case 500: {
