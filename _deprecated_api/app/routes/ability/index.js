...
const apiLegend = al.createLegend(
  'Ability',
...
apiLegend.register(
  ...
  'GET',
  '/ping',
  'This endpoint is used to ping the Ability Module API router',
  apiInfo.args.ping,
  apiInfo.rval.ping,
...
apiInfo.args.help = [
  {
    ...
    type: '~boolean',
    desc:
      'An optional boolean to request a pretty HTML page of the Ability API doc'
  }
]
apiInfo.rval.help = [
  {
...
apiLegend.register(
  ...
  'GET',
  '/help',
  'This endpoint sends the client documentation on API module (Ability)',
  apiInfo.args.help,
  apiInfo.rval.help,
  function (request, response) {
    const handlerTag = { src: '(get) /api/ability/help' }
    ...
    try {
      ...
      // Send the API doc
      const output = pretty
        ? apiLegend.getDoc(true)
        : rf.asCommonStr(true, apiLegend.getDoc(false))
      response
        .send(output)
        .status(200)
        .end()
...
apiInfo.rval.getAll = [
  {
    condition: 'On success',
    desc:
      'a code 200, and a full list of abilities assignable to clearance levels'
  },
  {
  ...
  },
  {
    ...
    desc:
      'a code 499, and an error-formatted JSON object detailing the expired session issue'
  },
  {
...
apiLegend.register(
  ...
  'GET',
  '/getAll',
  'Gets a full list of abilities assignable to clearance levels',
  apiInfo.args.getAll,
  apiInfo.rval.getAll,
  function (request, response) {
    const handlerTag = { src: '(get) /api/ability/getAll' }
    const sessionID =
...
router.use(function (request, response) {
  const handlerTag = { src: '/ability/notfound' }
...
router.use(function (request, response) {
  const handlerTag = { src: '/ability/error' }
